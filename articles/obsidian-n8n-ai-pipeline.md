...
:::message
**検証環境**: Obsidian + n8n（セルフホスト）+ Supabase（pgvector）+ OpenAI（埋め込み）+ Anthropic API（要約・分類）。2026年5月時点。月次コストは数千円以内。各ツールの仕様・価格は変動するので、ポートやモデル・価格は[公式](https://supabase.com/docs/guides/ai)で最新を確認してほしい。
:::
...
```mermaid
graph TB
  E["WF-EMBED<br/>ノート→ベクトル化→Supabase"] --> V[("Supabase pgvector")]
  V --> M["WF-MOC<br/>クラスタリングでMOC生成"]
  V --> R["WF-REVIEW<br/>類似ノートの復習提示"]
  V --> T["WF-TRIAGE<br/>分類・タグ付け"]
  V --> B["WF-DAILY-BRIEF<br/>情報ブリーフィング"]
```
...
-- ~10万行までは ivfflat / lists=100 で十分。
-- 現行の推奨は HNSW（再現率・更新耐性で有利）。データが増えるなら下記に切り替える：
--   CREATE INDEX ON obsidian_embeddings USING hnsw (embedding vector_cosine_ops);
...
:::message
埋め込みモデルは `text-embedding-3-small`（次元数1536）を使っている。`ada-002` より精度が高く、`3-large` の**約1/6.5のコスト**（$0.02 vs $0.13 / 1M tokens）。月数十円で収まる。インデックスは現在 ivfflat だが、**Supabase 含め現行の推奨は HNSW**。行数が増えたら HNSW への切り替えを検討してほしい。
:::
...
:::message
（数字の出どころ）HOT layer サイズは対象ファイル群のバイト数合計、コンテキスト使用率はセッション開始直後の使用トークン÷上限トークン。いずれも俺の環境での実測値で、プロジェクト構成によって変わる。
:::
...
:::message alert
**Dockerネットワークの罠。** n8n はコンテナ内で動いている。`localhost` で Obsidian に接続しようとすると、コンテナ自身を指すため接続できない。
:::

Obsidian の Local REST API プラグインは**既定では HTTPS の 27124 番**で待ち受ける。HTTP の 27123 番は**既定で無効**で、設定で明示的に有効化したときだけ使える。俺は内部ネットワーク前提で HTTP を有効化して 27123 を使っているが、デフォルトのままなら 27124（HTTPS）を指す必要がある。
...
// 既定では 27124(HTTPS)。HTTP(27123) はプラグイン設定で有効化した場合のみ
const obsidianUrl = 'http://host.docker.internal:27123';
```

:::message
Docker Desktop なら `host.docker.internal` がそのまま使えるが、**素の Linux + Docker では `--add-host=host.docker.internal:host-gateway`** を付けないと名前解決できない。
:::
