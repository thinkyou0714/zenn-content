...
:::message
**検証環境**: 自宅NUCマシン（AMD Radeon / RAM 32GB）で n8n をセルフホスト（2026年5月時点）。月額コストはほぼ電気代のみ。n8n の Code ノード制約や API 仕様はバージョン・環境変数で変わるので、後述の注意書きと[公式ドキュメント](https://docs.n8n.io/)を併読してほしい。
:::
...
// v2.0 以降の既定では process.env がブロックされる → $env を使う
const key = $env.ANTHROPIC_API_KEY ?? '';

// グローバルの URL は未定義 → require で取得
// ※ require('url') 自体が既定では不許可。NODE_FUNCTION_ALLOW_BUILTIN の設定が前提
const { URL: _URL } = require('url');
const url = new _URL('https://api.example.com/path');
```

:::message alert
n8n の Code ノードのサンドボックス制約は**環境依存**だ。`process.env` は v2.0 以降の既定でブロックされる（`$env` を使う）。グローバルの `URL` も未定義。さらに `require('url')` のような組み込みモジュールも**既定では不許可**で、`NODE_FUNCTION_ALLOW_BUILTIN`（外部 npm は `NODE_FUNCTION_ALLOW_EXTERNAL`）を設定して初めて使える。セルフホストで自分が許可しているから動くだけで、**クラウド版や未設定環境ではこのコードは動かない**点に注意。
:::
...
```mermaid
graph TB
  L1["Layer 1: 収集レイヤー<br/>情報を集める"]
  L2["Layer 2: 処理レイヤー<br/>情報を変換・分析する"]
  L3["Layer 3: 出力レイヤー<br/>外部に出力する（人間承認あり）"]
  L1 --> L2 --> L3
```
...
:::message
（数字の出どころ）Anthropic API 費用は管理コンソールの月次使用量、電気代は NUC の実測消費電力×電力単価から算出した俺の環境での実績値。「55本」は有効化されている n8n ワークフロー数。トークン量や稼働状況で変動する。
:::
